...
class TaskForDayHolder(private val binding: ItemTaskBinding,
                       private var onItemclick: (TaskForDay) -> Unit) : RecyclerView.ViewHolder(binding.root) {
    ...
    fun OnBind(taskForDay: TaskForDay) {
        with(binding){
            ...
            verticalMenuButton.setOnClickListener {
                onItemclick(taskForDay)
            }
        }
    }
}